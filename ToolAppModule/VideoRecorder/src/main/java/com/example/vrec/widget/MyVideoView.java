...
public class MyVideoView extends SurfaceView implements MyMediaController.MediaPlayerControl,
    ...
    private MyMediaController mmMyMediaController;
...
//                    canSeekForward();
    ...
    private void openVideo() {
        ...
        try {
            ...
            LoggerUtils.loge("Unable to open content: " + mUri + "," + ex);
            return;
    ...
    public void setMediaController(MyMediaController controller) {
        if (mmMyMediaController != null) {
            mmMyMediaController.hide();
        }
        mmMyMediaController = controller;
        attachMediaController();
    }

    private void attachMediaController() {
        if (mMediaPlayer != null && mmMyMediaController != null) {
            mmMyMediaController.setMediaPlayer(this);
            ViewGroup anchorView = (ViewGroup) (this.getParent() instanceof View ? (View) this
                    .getParent() : this);
            mmMyMediaController.setAnchorView(anchorView);
            mmMyMediaController.setEnabled(mIsPrepared);
        }
    }
    ...
    MediaPlayer.OnPreparedListener mPreparedListener = new MediaPlayer.OnPreparedListener() {
        public void onPrepared(MediaPlayer mp) {
            ...
            if (mmMyMediaController != null) {
                mmMyMediaController.setEnabled(true);
            }
            mVideoWidth = mp.getVideoWidth();
            mVideoHeight = mp.getVideoHeight();
            if (mVideoWidth != 0 && mVideoHeight != 0) {
    ...
    private MediaPlayer.OnBufferingUpdateListener mBufferingUpdateListener = new MediaPlayer
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        if (mIsPrepared && mMediaPlayer != null && mmMyMediaController != null) {
    ...
    private void toggleMediaControlsVisiblity() {
        if (mmMyMediaController.isShowing()) {
            mmMyMediaController.hide();
        } else {
            mmMyMediaController.show();
        }
    ...
    public void start() {
        if (mMediaPlayer != null && mIsPrepared) {
            mMediaPlayer.start();
            mmMyMediaController = new MyMediaController(getContext());
            mmMyMediaController.updatePausePlay();
            mStartWhenPrepared = false;
        } else {
