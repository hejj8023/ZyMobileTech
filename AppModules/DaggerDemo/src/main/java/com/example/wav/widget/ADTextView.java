...
public class ADTextView extends View {
    ...
    private void obtainStyledAttrs(AttributeSet attrs) {
        ...
        mFrontTextSize = (int) array.getDimension(R.styleable.ADTextView_ad_text_front_size,
                UiUtils.Sp2Px(getContext(), 15));
        mContentTextSize = (int) array.getDimension(R.styleable.ADTextView_ad_text_content_size,
                UiUtils.Sp2Px(getContext(), 15));
        array.recycle();
    }
    ...
    protected void onDraw(Canvas canvas) {
        if (mTexts != null) {
            ...
            canvas.drawText(back, 0, back.length(), (indexBound.right - indexBound.left) + 20,
                    mY, mPaintContent);
            canvas.drawText(font, 0, font.length(), 10, mY, mPaintFront);
            //移动到中间
            if (!isPaused && mY <= getMeasuredHeight() / 2 - (indexBound.top + indexBound.bottom)
                    / 2) {
                isMove = false;
                isPaused = true;
    ...
    private int measureHeight(int heightMeasureSpec) {
        ...
        if (mode == MeasureSpec.EXACTLY) {
            ...
            int mContentTextHeight = (int) (mPaintContent.descent() - mPaintContent.ascent());
            //内容文字字高
