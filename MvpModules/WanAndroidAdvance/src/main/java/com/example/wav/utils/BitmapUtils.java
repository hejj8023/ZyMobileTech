        ...
        if (bitmap != null && !bitmap.isRecycled()) {
            try {
                ...
                if (byteArrayOutputStream.toByteArray() == null) {
                    LoggerUtils.loge("bitmap2Bytes byteArrayOutputStream toByteArray=null");
                }
                return byteArrayOutputStream.toByteArray();
            } catch (Exception e) {
                LoggerUtils.loge("bitmap2Bytes exception " + e.getMessage());
            } finally {
            ...
            LoggerUtils.loge("bitmap2Bytes bitmap == null or bitmap.isRecycled()");
            return null;
        }
    }
        ...
        if (datas != null && datas.length > byteCount) {
            ...
            if (outputStream != null) {
                ...
                if (outputStreamByte.length > byteCount) {
                    LoggerUtils.loge("compressBitmap cannot compress to " + byteCount + "," +
                            " after compress size=" + outputStreamByte.length);
                }
